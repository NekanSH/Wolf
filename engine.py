...
class WolfEngine:
    def __init__(self):
        ...
        self.btc_up = True; self.btc_trend = "UP"
    ...
    def _check_entry(self, symbol: str) -> tuple[str|None, str]:
        ...
        if self.btc_trend == "DOWN" and symbol in cfg.SHORT_SYMBOLS and hour in cfg.SESSION_SHORT_HOURS:
            return "SHORT", ""

        if self.btc_trend == "UP" and symbol in cfg.LONG_SYMBOLS and hour in cfg.SESSION_LONG_HOURS:
            return "LONG", ""
    ...
    async def on_candle(self, symbol, cd):
            ...
            if st.ema_fast.ready and st.ema_mid.ready:
                ...
                if len(self._btc_prices) >= 7:
                    ...
                    if p > cfg.BTC_VELOCITY_MIN and r < p*cfg.BTC_VELOCITY_DECAY:
                        vel_weak = True
                if self.btc_up and ema_rising and not vel_weak:   self.btc_trend="UP"
                elif self.btc_up:                                 self.btc_trend="WEAK"
        ...
        for pos in self.open:
            ...
            if not reason:
                ...
                if pos.max_pnl >= t_act:
                    ...
                    if cur <= pos.trailing_floor: reason = f"TRAILING_{pos.symbol[:3]}"
        ...
        with open(cfg.CSV_SIGNALS, "a", newline="") as f:
            csv.writer(f).writerow([int(time.time()*1000), symbol, side, price, round(vol_r,2), self.btc_trend, hour])
    ...
    def _close(self, pos, price, held, reason):
        ...
        with open(cfg.CSV_TRADES, "a", newline="") as f:
            csv.writer(f).writerow([pos.symbol, pos.side, round(pos.entry_price,6), round(pos.exit_price,6),
                                    round(pos.pnl_pct,4), round(pos.pnl_usdt,4), pos.leverage, held, pos.entry_vol,
                                    round(pos.max_pnl,4), reason, pos.btc_regime, pos.entry_hour])
    ...
    def snapshot(self):
        ...
        for s in cfg.SYMBOLS:
            ...
            session = f"{side}✓ {hour:02d}UTC" if side else f"pause {hour:02d}UTC"
            result.append({
                "symbol":s, "price":st.last_price, "vol":vol, "ready":st.ready, 
                "candles":st.candle_count, "session":session
            })
        return result
    ...
    def stats(self):
        ...
        base = {
            "trades":0,"wr":0,"pnl":0,"avg":0,"best":0,"worst":0,"open":len(self.open),
            "longs":self.long_count,"shorts":self.short_count,"btc_trend":self.btc_trend,
            "block_reasons":dict(self.block_reasons),"exit_reasons":dict(reasons),"loss_streak":self._consecutive_losses
        }
        if not cl: return base
        w = [p for p in cl if p.pnl_usdt > 0]
        base.update({
            "trades":len(cl),"wr":round(len(w)/len(cl)*100,1),"pnl":round(self.total_pnl,2),
            "avg":round(self.total_pnl/len(cl),2),"best":round(max(p.pnl_pct for p in cl),2),
